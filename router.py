...
class TaskComplexityClassifier:
    ...
    def classify(self, messages: list[Dict[str, str]]) -> Tuple[bool, str]:
        ...
        if found_keywords:
            ...
            logger.info(f"Task classified as complex: {reason}")
            return True, reason
        ...
        logger.info("Task classified as simple")
...
class SmartRouter:
    ...
    def __init__(self):
        ...
        self.local_client = LocalModelClient(
            self.config.models["local"].model_dump()
        )
    ...
    async def route(
        ...
        **kwargs
        ...
        if strategy == "always_local":
            ...
            return await self._smart_routing(messages, temperature, max_tokens, **kwargs)
        
        elif strategy == "speculative_decoding":
            return await self._speculative_decoding(messages, temperature, max_tokens, **kwargs)
        
        else:
            logger.warning(f"Unknown strategy: {strategy}, falling back to smart_routing")
            return await self._smart_routing(messages, temperature, max_tokens, **kwargs)
    ...
    async def _speculative_decoding(
        ...
        **kwargs
        ...
        if not self.config.speculative_decoding.enabled:
            logger.info("Speculative decoding disabled, using smart routing")
            return await self._smart_routing(messages, temperature, max_tokens, **kwargs)
        
        logger.info("Using speculative decoding")
        
        # Generate draft from local model
        draft_response = await self._use_local(messages, temperature, max_tokens, **kwargs)
        ...
        logger.info("Speculative decoding: returning draft from local model")
